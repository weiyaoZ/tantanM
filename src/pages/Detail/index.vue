        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转：从 A 路由跳转到 B 路由，这里在加入购物车，进行路由跳转之前，发请求把
                购买的产品的信息通过请求的形式通知服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    ...
    // 给子组件的数据【进行计算以及赋默认值，让 skuImageList 至少是一个数组】
    skuImageList() {
  ...
  methods: {
    ...
    // 表单元素修改产品个数
    changeSkuNum(event) {
      // 用户输入进来的文本 * 1【若包含非数字的文本，值将为 NAN】
      let value = event.target.value * 1;
      // 如果用户输入进来的非法，出现 NAN 或者 小于1
      if (isNaN(value) || value < 1) {
        this.skuNum = 1;
      } else {
        // 正常大于 1 【大于 1 整数不能出现小数】
    ...
    async addShopCart() {
      ...
      /* 
        当前这里派发一个 action，也向服务器发送请求，判断加入购物车是成功还是失败，进行相应的操作
      */
      try {
        ...
        // 路由跳转
        // 一些简单的数据（skuNum）通过 query 形式传递给下一个组件
        // 产品信息的数据【比较复杂：skuIfo】，通过会话存储（不持久化，会话关闭数据消失）
        // 本地存储|会话存储，一般存储的是字符串
          ...
          .chooseArea {
            ...
            dl {
              ...
              dd {
                ...
                &.active {
                  color: rgb(167, 16, 16);
                  border: 1px solid rgb(144, 28, 10);
                }
              }
            }
          }
